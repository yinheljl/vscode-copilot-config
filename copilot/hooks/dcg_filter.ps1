...
function Approve-Hook {
    $out = @{ permissionDecision = "allow" } | ConvertTo-Json -Compress
    Write-Output $out
    exit 0
}
...
# Copilot preToolUse: toolName + toolArgs (JSON string)
$command = $null
...
# toolArgs is a JSON string containing {"command": "..."}
if ($event.PSObject.Properties.Name -contains "toolArgs") {
    $argsStr = [string]$event.toolArgs
    if ($argsStr) {
        try {
            $argsObj = $argsStr | ConvertFrom-Json -ErrorAction Stop
            if ($argsObj.PSObject.Properties.Name -contains "command") {
