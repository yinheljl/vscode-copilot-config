...
- **实现方**：[@Dicklesworthstone](https://github.com/Dicklesworthstone)（个人维护，已验证 release tag v0.5.6/2026-05，活跃中）
...
- **本仓库的角色**：restore 脚本检测 dcg 是否安装；未装时弹 `[y/N]` 确认，显式使用 `-AutoInstallDcg` / `--auto-install-dcg` 时会安装或刷新。若当前 agent / 平台支持 hooks，就部署对应的低噪音 hook；不支持时只保留软层 SKILL。
