...
**局限**：
- 软规则属于 prompt 层，模型可能在某些极端情况下绕过（如上下文严重压缩、`--full-auto` 模式）

### 硬层（hook，由 `restore` 脚本自动安装）

[`zhexulong/codex-safeguard`](https://github.com/zhexulong/codex-safeguard) —— Codex 原生 PreToolUse hook：

- 通过 npm 全局安装（`npm install -g codex-safeguard`）
- `restore.ps1` / `restore.sh` 会自动部署 `~/.codex/hooks.json`，注册 PreToolUse hook
- 命令在**进入 shell 之前**被拦截，不依赖模型自觉性
- 检测到危险模式直接 `deny`，无法绕过（除非用户手动卸载）

如果本机未安装 Node.js / npm，硬层会被自动跳过并打印警告，软层仍然生效。
...
直接运行仓库根目录的 `update.ps1` / `update.sh`（会自动 `git pull` + `restore` + 安装 codex-safeguard hook），或仅运行 `restore.ps1` / `restore.sh`。
